...
Минимальный проект Telegram Mini App + Telegram bot для заявок на учебные работы. Архитектура взята по техническим паттернам из `D:\uchi-bot`, а продуктовая часть здесь своя: услуги, заявки, управление и отзывы.
...
- `/orders` открывает Mini App на странице заявок

Когда пользователь оставляет заявку, backend отправляет всем пользователям с `is_admin=true` сообщение в Telegram. После перевода заявки в `done` пользователь получает кнопку для отзыва.
